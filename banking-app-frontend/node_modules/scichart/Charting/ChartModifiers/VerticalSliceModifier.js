...
var VerticalSliceModifier = /** @class */ (function (_super) {
    ...
    VerticalSliceModifier.prototype.modifierMouseMove = function (args) {
        ...
        if (this.rolloverLineAnnotation.isDraggingStarted) {
            ...
            var x = isVertical ? args.mousePoint.y : args.mousePoint.x;
            var xAxis = this.xAxis;
            var coordCalc = xAxis.getCurrentCoordinateCalculator();
            switch (this.xCoordinateMode) {
                case AnnotationBase_1.ECoordinateMode.Pixel: {
                    var startOffset = (isVertical ? xAxis.viewRect.top : xAxis.viewRect.left);
                    var viewSize = (isVertical ? xAxis.viewRect.height : xAxis.viewRect.width);
                    this.x1 = Math.min(Math.max(0, (x - startOffset) / DpiHelper_1.DpiHelper.PIXEL_RATIO), viewSize / DpiHelper_1.DpiHelper.PIXEL_RATIO);
                    break;
                }
                case AnnotationBase_1.ECoordinateMode.Relative: {
                    var startOffset = isVertical ? xAxis.viewRect.top : xAxis.viewRect.left;
                    var newX = (x - startOffset) / coordCalc.viewportDimension;
    ...
    VerticalSliceModifier.prototype.calculateXPosition = function () {
        ...
        if (xAxis) {
            ...
            var isVertical = this.isVerticalChart();
            var startOffset = isVertical ? xAxis.viewRect.top : xAxis.viewRect.left;
            var DPI = this.isSvgOnlyProperty ? DpiHelper_1.DpiHelper.PIXEL_RATIO : 1;
            // @ts-ignore
            var x = this.rolloverLineAnnotation.getCoordinate(this.x1Property, xCalc, this.xCoordinateModeProperty);
            var adjustedX = x * DPI + startOffset;
            // pick a y value in the view area
            var y1v = isVertical
                ? this.parentSurface.seriesViewRect.width / 2
                : this.parentSurface.seriesViewRect.height / 2;
            this.mousePoint = isVertical ? new Point_1.Point(y1v, adjustedX) : new Point_1.Point(adjustedX, y1v);
        }
    };

...
var NonUniformHeatmapDataSeries = /** @class */ (function (_super) {
     ...
     */
    NonUniformHeatmapDataSeries.prototype.getXCellSizes = function (getCoordFn) {
        if (!this.xCellSizesProperty) {
            this.xCellSizesProperty = this.calculateCellSizes(this.xCellOffsetsProperty, getCoordFn);
        }
     ...
     * Can be an array of numbers or a mapping function {@link TCellSizeMapper}
     */
    NonUniformHeatmapDataSeries.prototype.getYCellSizes = function () {
        if (!this.yCellSizesProperty) {
            this.yCellSizesProperty = this.calculateCellSizes(this.yCellOffsetsProperty, function (x) { return x; });
        }
    ...
    NonUniformHeatmapDataSeries.prototype.notifyDataChanged = function (updateType, data) {
        ...
        if (updateType === IDataSeries_1.EDataChangeType.Append) {
            ...
            if (this.xCellOffsetsGeneratorFunction) {
                ...
                // set to undefined to regenerate on the next getXCellSizes() call
                this.xCellSizesProperty = undefined;
            }
            if (this.yCellOffsetsGeneratorFunction) {
                ...
                // set to undefined to regenerate on the next getYCellSizes() call
                this.yCellSizesProperty = undefined;
            }
     ...
     * @returns an array with cell sizes to heatmap size ratios
     */
    NonUniformHeatmapDataSeries.prototype.calculateCellSizes = function (cellOffsets, getCoordFn) {
        if (!cellOffsets || cellOffsets.length === 0) {
            return [];
        }
        var count = cellOffsets.length;
        var heatmapRange = getCoordFn(cellOffsets[count - 1]) - getCoordFn(cellOffsets[0]);
        var cellSizeRatios = [];
        for (var i = 0; i < count - 1; ++i) {
            var diff = getCoordFn(cellOffsets[i + 1]) - getCoordFn(cellOffsets[i]);

...
var setLabel = function (text, styleId, label) {
    var key = makeCacheKey(text, styleId);
    // THIS is probably not needed as this should not happen after the change in LabelProviderBase2D.
    // Moreover it it happens having this code results in no showing labels referencing the removed texture
    // // Dispose old label texture if it exists to prevent memory leak
    // const existingLabel = labelCacheByTextAndStyle.get(key);
    // if (existingLabel && existingLabel.bitmapTexture) {
    //     try {
    //         deleteSafe(existingLabel.bitmapTexture);
    //     } catch (error: any) {
...
var pruneCache = function () {
    if (Date.now() > lastPrune + minAge && labelCacheByTextAndStyle.size > maxSize) {
        try {
            ...
            if (toRemove > maxSize) {
                performanceWarnings_1.performanceWarnings.labelCacheTooSmall.warn();
            }
            lastPrune = Date.now();
        }
        catch (err) {
...
var resetCache = function () {
    // Dispose all textures first to prevent memory leak
    labelCacheByTextAndStyle.forEach(function (label) {
        if (label && label.bitmapTexture) {
            try {
                (0, Deleter_1.deleteSafe)(label.bitmapTexture);
            }
            catch (error) {
...
exports.labelCache = {
    ...
    getSize: getSize,
    /** Get the minimum time (in ms) between cache pruning.  Default 200ms
     * This prevents recently used labels from being pruned, or removed when style is freed
     */
    getMinAge: getMinAge,
    /** Set the minimum time (in ms) between cache pruning.  Default 200ms
     * This prevents recently used labels from being pruned, or removed when style is freed
     */
    setMinAge: setMinAge,

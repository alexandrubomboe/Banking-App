...
var BaseStackedCollection = /** @class */ (function (_super) {
    ...
    BaseStackedCollection.prototype.getNativeYValues = function () {
        throw Error("getNativeYValues() method is not supported for BaseStackedCollection");
    };
    /** @inheritDoc */
    BaseStackedCollection.prototype.getYRange = function (xVisibleRange, isXCategoryAxis) {
        ...
        this.updateAccumulatedVectors(); // Just in case
        var maxRange;
        this.getVisibleSeries().forEach(function (rs) {
            if (rs.accumulatedValues.size() === _this.getNativeXValues().size()) {
                var range = (0, BaseDataSeries_1.getWindowedYRange)(_this.webAssemblyContext, _this.getNativeXValues(), rs.getAccumulatedValues(_this.isRunningDataAnimation), xVisibleRange, true, isXCategoryAxis, rs.dataSeries.dataDistributionCalculator.isSortedAscending, true, // Potential optimisation, if we know for certain data does not containsNaN, we can use SIMD ops
                _this.yRangeMode === YRangeMode_1.EYRangeMode.Visible ? SearchMode_1.ESearchMode.RoundUp : SearchMode_1.ESearchMode.RoundDown, _this.yRangeMode === YRangeMode_1.EYRangeMode.Visible ? SearchMode_1.ESearchMode.RoundDown : SearchMode_1.ESearchMode.RoundUp);
                if (range) {
    ...
    BaseStackedCollection.prototype.resolveAutoColors = function (index, maxSeries, theme) {
        throw Error("resolveAutoColors method is not supported for BaseStackedCollection.  Call on collection items individually");
    };
    /** @inheritDoc */
    ...
    BaseStackedCollection.prototype.beforeAnimationStart = function () {
        ...
        (0, copyVector_1.copyDoubleVector)(this.accumulatedValues0, this.accumulatedFinalAnimationValues0, this.webAssemblyContext);
        this.getVisibleSeries().forEach(function (rs) {

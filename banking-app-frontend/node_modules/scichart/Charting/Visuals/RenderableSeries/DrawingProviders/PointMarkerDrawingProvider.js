...
var PointMarkerDrawingProvider = /** @class */ (function (_super) {
    ...
    PointMarkerDrawingProvider.prototype.draw = function (renderContext, renderPassData) {
        ...
        if (this.parentSeries.hasPointMarkerPaletteProvider()) {
            ...
            // renderContext.drawTexture(spriteTexture.getTexture(), 10, 10, 256, 256);
            // END DEBUG CODE
        }
    ...
    PointMarkerDrawingProvider.prototype.overridePaletteProviderColors = function (rs, xValue, yValue, index, opacity, metadata, otherValues) {
        if (rs.hasPointMarkerPaletteProvider()) {
            var pointMarkerPaletteProvider = rs.paletteProvider;
            var colors = pointMarkerPaletteProvider.overridePointMarkerArgb(xValue, yValue, index, opacity, metadata, otherValues);
            if (colors)
                return colors;
        }

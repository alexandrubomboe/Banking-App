...
var StackedColumnSeriesHitTestProvider = /** @class */ (function (_super) {
    ...
    StackedColumnSeriesHitTestProvider.prototype.hitTest = function (x, y) {
        ...
        if (nearestPointIndex >= 0) {
            ...
            var accumTop = accumulatedValues.get(nearestPointIndex);
            // When separatePositiveNegativeStacks is active, bottomAccumulatedValues is populated
            // and gives the true bottom edge of each column. Fall back to the legacy computation
            // (accumTop - yValue) when it is not available.
            var bottomAccumulatedValues = this.parentSeries.bottomAccumulatedValues;
            var separateStacking = bottomAccumulatedValues.size() > 0;
            var accumBottom = separateStacking
                ? bottomAccumulatedValues.get(nearestPointIndex)
                : accumTop - yValue;
            var topCoord = yCoordinateCalculator.getCoordinate(accumTop);
            var bottomCoord = yCoordinateCalculator.getCoordinate(accumBottom);
            ...
            // Place the rollover marker at the outer tip of the column.
            // For negative values with separate stacking, the outer tip is the bottom edge.
            if (separateStacking && yValue < 0) {
                hitTestInfo.yValue = accumBottom;
            }
    ...
    StackedColumnSeriesHitTestProvider.prototype.hitTestXSlice = function (x, y) {
        ...
        if (nearestPointIndex >= 0) {
            ...
            var yValue = yNativeValues.get(nearestPointIndex);
            var bottomAccumulatedValues = this.parentSeries.bottomAccumulatedValues;
            if (bottomAccumulatedValues.size() > 0 && yValue < 0) {
                hitTestInfo.yValue = bottomAccumulatedValues.get(nearestPointIndex);
            }
            // Hit-test on StackedColumnSeries returns Y accumulated, Y1 original
            hitTestInfo.y1Value = yNativeValues.get(nearestPointIndex);
        }

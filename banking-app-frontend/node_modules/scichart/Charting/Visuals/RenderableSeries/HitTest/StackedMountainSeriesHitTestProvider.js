...
var StackedMountainSeriesHitTestProvider = /** @class */ (function (_super) {
    ...
    StackedMountainSeriesHitTestProvider.prototype.hitTest = function (x, y) {
        ...
        if (nearestPointIndex >= 0) {
            var bottomAccumulatedValues_1 = this.parentSeries.bottomAccumulatedValues;
            var separateStacking_1 = bottomAccumulatedValues_1.size() > 0;
            var getTopLine = function (index) { return accumulatedValues.get(index); };
            var getBottomLine = function (index) {
                return separateStacking_1
                    ? bottomAccumulatedValues_1.get(index)
                    : accumulatedValues.get(index) - dataSeries.getNativeYValues().get(index);
            };
            ...
            // Place the rollover marker at the outer tip of the band.
            // For negative values with separate stacking, the outer tip is the bottom edge.
            if (separateStacking_1 && dataSeries.getNativeYValues().get(nearestPointIndex) < 0) {
                hitTestInfo.yValue = bottomAccumulatedValues_1.get(nearestPointIndex);
            }
    ...
    StackedMountainSeriesHitTestProvider.prototype.hitTestXSlice = function (x, y) {
        ...
        if (nearestPointIndex >= 0) {
            ...
            if (bottomAccumulatedValues.size() > 0 && yNativeValues.get(nearestPointIndex) < 0) {

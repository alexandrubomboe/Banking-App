...
var SciChartSurfaceCore_1 = require("../SciChartSurfaceCore");
...
var SciChartPieSurface = /** @class */ (function (_super) {
    ...
    function SciChartPieSurface(canvases, options) {
        ...
        _this.labelStyleProperty = {
            fontSize: 14,
            fontFamily: (0, font_1.getDefaultFontName)(),
            color: "#1e323d",
            fontWeight: "bold"
        };
        ...
        _this.cornerRadiusProperty = (_k = options === null || options === void 0 ? void 0 : options.cornerRadius) !== null && _k !== void 0 ? _k : _this.cornerRadiusProperty;
        _this.legend.showLegend = (_l = options === null || options === void 0 ? void 0 : options.showLegend) !== null && _l !== void 0 ? _l : _this.legend.showLegend;
        _this.legend.animate = (_m = options === null || options === void 0 ? void 0 : options.animateLegend) !== null && _m !== void 0 ? _m : _this.legend.animate;
        _this.legend.showCheckboxes = (_o = options === null || options === void 0 ? void 0 : options.showLegendCheckBoxes) !== null && _o !== void 0 ? _o : _this.legend.showCheckboxes;
        _this.legend.showSeriesMarkers = (_p = options === null || options === void 0 ? void 0 : options.showLegendSeriesMarkers) !== null && _p !== void 0 ? _p : _this.legend.showSeriesMarkers;
        _this.paddingProperty = (_q = options === null || options === void 0 ? void 0 : options.padding) !== null && _q !== void 0 ? _q : _this.paddingProperty;
        ...
        _this.labelProvider = (_r = options === null || options === void 0 ? void 0 : options.labelProvider) !== null && _r !== void 0 ? _r : new PieLabelProvider_1.PieLabelProvider();
        _this.valueModeProperty = (_s = options === null || options === void 0 ? void 0 : options.valueMode) !== null && _s !== void 0 ? _s : _this.valueModeProperty;
        return _this;
    }
    /**
    ...
    Object.defineProperty(SciChartPieSurface.prototype, "holeRadius", {
        ...
        set: function (holeRadius) {
            if (this.holeRadiusProperty !== holeRadius) {
    ...
    Object.defineProperty(SciChartPieSurface.prototype, "holeRadiusSizingMode", {
        ...
        set: function (holeRadiusSizingMode) {
            if (this.holeRadiusSizingModeProperty !== holeRadiusSizingMode) {
    ...
    Object.defineProperty(SciChartPieSurface.prototype, "canvasBorder", {
        ...
        set: function (value) {
            if (this.canvasBorderProperty !== value) {
                ...
                this.updateLegendMargin();
                this.notifyPropertyChanged(constants_1.PROPERTY.CANVAS_BORDER);
            }
        },
        enumerable: false,
        configurable: true
    ...
    Object.defineProperty(SciChartPieSurface.prototype, "seriesSpacing", {
        /**
         * Gets or sets spacing between the segments
         */
        get: function () {
    ...
    Object.defineProperty(SciChartPieSurface.prototype, "valueMode", {
        ...
        /** Whether to show labels as percentages, or raw values.  Default to percentages */
        set: function (value) {
            if (this.valueModeProperty !== value) {
    ...
    Object.defineProperty(SciChartPieSurface.prototype, "labelRadiusAdjustment", {
         ...
         */
        set: function (value) {
            if (this.labelRadiusProperty !== value) {
                this.labelRadiusProperty = value;
                this.notifyPropertyChanged(constants_1.PROPERTY.LABEL_RADIUS);
            }
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(SciChartPieSurface.prototype, "cornerRadius", {
        /**
         * Gets or sets the corner radius for pie/donut segments.
         * Default 0 (no rounded corners).
         */
        get: function () { return this.cornerRadiusProperty; },
        set: function (value) {
            if (this.cornerRadiusProperty !== value) {
                this.cornerRadiusProperty = value;
    ...
    SciChartPieSurface.prototype.draw = function () {
        var _this = this;
        // TODO check whether this requires some kind of updates suspend
    ...
    SciChartPieSurface.prototype.drawChart = function (animationProgress) {
        ...
        segments.forEach(function (el, index) {
            ...
            var dAttribute = getRoundedPiePath(xCoord, yCoord, outerRadius * el.radiusAdjustment, innerRadius, angleFrom, angleTo, el.shift + _this.seriesSpacing, _this.cornerRadiusProperty);
...
}(SciChartSurfaceCore_1.SciChartSurfaceCore));
exports.SciChartPieSurface = SciChartPieSurface;
/**
 * @ignore
 * @description Create rounded sector path string (Donut or Pie)circle. This runs per-segment.
 ...
 */
var getRoundedPiePath = function (x, y, outerR, innerR, a1, a2, delta, cornerRadius) {
    ...
    if (angleDiff >= 360) {
        return "M".concat(x + outerR, " ").concat(y, " ")
            + "A".concat(outerR, " ").concat(outerR, " 0 1 1 ").concat(x - outerR, " ").concat(y, " ")
            + "A".concat(outerR, " ").concat(outerR, " 0 1 1 ").concat(x + outerR, " ").concat(y, " ")
            + "M".concat(x + innerR, " ").concat(y, " ")
            + "A".concat(innerR, " ").concat(innerR, " 0 1 0 ").concat(x - innerR, " ").concat(y, " ")
            + "A".concat(innerR, " ").concat(innerR, " 0 1 0 ").concat(x + innerR, " ").concat(y, "Z");
    }
    var startRad = a1 * DEG_TO_RAD;
    var endRad = a2 * DEG_TO_RAD;
    var totalAngleRad = Math.abs(endRad - startRad);
    var bigArc = angleDiff > 180 ? 1 : 0;
    // cornerRadius constraints
    var cr = Math.min(cornerRadius, (outerR - innerR) / 2); // for slices thinner than cr * 2
    var sinHalf = Math.sin(totalAngleRad / 2);
    cr = Math.min(cr, outerR * sinHalf / (1 + sinHalf));
    var safeRadiusOut = outerR - cr;
    var alphaOut = safeRadiusOut > 0 ? Math.asin(cr / safeRadiusOut) : 0;
    if (cr <= 0) {
        var a1_1 = startRad;
        var a2_1 = endRad;
        var x1Out = x + outerR * Math.cos(a1_1);
        var y1Out = y + outerR * Math.sin(a1_1);
        var x2Out = x + outerR * Math.cos(a2_1);
        var y2Out = y + outerR * Math.sin(a2_1);
        if (innerR > 0) {
            var x2In = x + innerR * Math.cos(a2_1);
            var y2In = y + innerR * Math.sin(a2_1);
            var x1In = x + innerR * Math.cos(a1_1);
            var y1In = y + innerR * Math.sin(a1_1);
            return "M".concat(x1Out, ",").concat(y1Out, " A").concat(outerR, ",").concat(outerR, " 0 ").concat(bigArc, ",1 ").concat(x2Out, ",").concat(y2Out, " L").concat(x2In, ",").concat(y2In, " A").concat(innerR, ",").concat(innerR, " 0 ").concat(bigArc, ",0 ").concat(x1In, ",").concat(y1In, " Z");
        }
        else {
            return "M".concat(x, ",").concat(y, " L").concat(x1Out, ",").concat(y1Out, " A").concat(outerR, ",").concat(outerR, " 0 ").concat(bigArc, ",1 ").concat(x2Out, ",").concat(y2Out, " Z");
        }
    }
    // rounded corners
    var outerStartAngle = startRad + alphaOut;
    var pOuterMainStart_x = x + outerR * Math.cos(outerStartAngle);
    var pOuterMainStart_y = y + outerR * Math.sin(outerStartAngle);
    var outerEndAngle = endRad - alphaOut;
    var pOuterMainEnd_x = x + outerR * Math.cos(outerEndAngle);
    var pOuterMainEnd_y = y + outerR * Math.sin(outerEndAngle);
    var distToRadialOuter = Math.sqrt(Math.pow((outerR - cr), 2) - Math.pow(cr, 2));
    var pRadialStartOuter_x = x + distToRadialOuter * Math.cos(startRad);
    var pRadialStartOuter_y = y + distToRadialOuter * Math.sin(startRad);
    var pRadialEndOuter_x = x + distToRadialOuter * Math.cos(endRad);
    var pRadialEndOuter_y = y + distToRadialOuter * Math.sin(endRad);
    var d = "M ".concat(pRadialStartOuter_x, ",").concat(pRadialStartOuter_y)
        + " A ".concat(cr, ",").concat(cr, " 0 0,1 ").concat(pOuterMainStart_x, ",").concat(pOuterMainStart_y)
        + " A ".concat(outerR, ",").concat(outerR, " 0 ").concat(bigArc, ",1 ").concat(pOuterMainEnd_x, ",").concat(pOuterMainEnd_y)
        + " A ".concat(cr, ",").concat(cr, " 0 0,1 ").concat(pRadialEndOuter_x, ",").concat(pRadialEndOuter_y);
    if (innerR > 0) { // has inner radius (is donut, not pie)
        var distToRadialInner = Math.sqrt(Math.pow((innerR + cr), 2) - Math.pow(cr, 2));
        var pRadialEndInner_x = x + distToRadialInner * Math.cos(endRad);
        var pRadialEndInner_y = y + distToRadialInner * Math.sin(endRad);
        var angOffsetIn = (innerR > 0) ? Math.asin(cr / (innerR + cr)) : 0;
        var pInnerMainEnd_x = x + innerR * Math.cos(endRad - angOffsetIn);
        var pInnerMainEnd_y = y + innerR * Math.sin(endRad - angOffsetIn);
        var pInnerMainStart_x = x + innerR * Math.cos(startRad + angOffsetIn);
        var pInnerMainStart_y = y + innerR * Math.sin(startRad + angOffsetIn);
        var pRadialStartInner_x = x + distToRadialInner * Math.cos(startRad);
        var pRadialStartInner_y = y + distToRadialInner * Math.sin(startRad);
        d += " L ".concat(pRadialEndInner_x, ",").concat(pRadialEndInner_y)
            + " A ".concat(cr, ",").concat(cr, " 0 0,1 ").concat(pInnerMainEnd_x, ",").concat(pInnerMainEnd_y)
            + " A ".concat(innerR, ",").concat(innerR, " 0 ").concat(bigArc, ",0 ").concat(pInnerMainStart_x, ",").concat(pInnerMainStart_y)
            + " A ".concat(cr, ",").concat(cr, " 0 0,1 ").concat(pRadialStartInner_x, ",").concat(pRadialStartInner_y, " Z");
    }
    else {
        d += " L ".concat(x, ",").concat(y, " Z");
    }
    return d;
};
/** @ignore */

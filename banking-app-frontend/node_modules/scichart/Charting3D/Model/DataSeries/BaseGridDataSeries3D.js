...
var BaseGridDataSeries3D = /** @class */ (function (_super) {
    ...
    function BaseGridDataSeries3D(webAssemblyContext, options) {
        ...
        // Copy yValues
        _this.setYValues(options === null || options === void 0 ? void 0 : options.yValues, options === null || options === void 0 ? void 0 : options.metadata);
        return _this;
    }
     ...
     */
    BaseGridDataSeries3D.prototype.getMetadataValues = function () {
        return this.metadataProperty;
    };
    /**
     * Sets a 2D array of YValues. Input is in the format YValues[z][x]
     * where z is 0 to zSize and X is 0 to xSize
     * Direct changes to this array will not be noticed by the chart.  Use setYValue or call notifyDataChanged() after an update
     * @param zValues
     */
    BaseGridDataSeries3D.prototype.setYValues = function (YValues, metadata) {
        ...
        if (metadata) {
            guardSameLengthYValuesAndMetadata(YValues, metadata);
     ...
     * @param metadata optional metadata for the data point.  The full metadata array must already exist
     */
    BaseGridDataSeries3D.prototype.setYValue = function (zIndex, xIndex, yValue, metadata) {
        this.yValuesProperty[zIndex][xIndex] = yValue;
        if (metadata) {
            this.validateIndexes(zIndex, xIndex);
            this.metadataProperty[zIndex][xIndex] = metadata;
        }
        this.notifyDataChanged();
    };
    /**
     ...
     */
    BaseGridDataSeries3D.prototype.getMetadataAt = function (zIndex, xIndex) {
        if (!this.metadataProperty) {
            return undefined;
        }
        this.validateIndexes(zIndex, xIndex);
        return this.metadataProperty[zIndex][xIndex];
    };
    /**
     * Gets the metadata matrix height
     */
    BaseGridDataSeries3D.prototype.getMetadataHeight = function () {
        if (!this.metadataProperty) {
            return this.zSizeProperty;
        }
     ...
     */
    BaseGridDataSeries3D.prototype.getMetadataWidth = function () {
        if (!this.metadataProperty) {
            return this.xSizeProperty;
        }
        return this.metadataProperty[0].length;
    };
    BaseGridDataSeries3D.prototype.setMetadata = function (metadata) {
        this.metadataProperty = metadata;
        this.notifyDataChanged();
    };
    BaseGridDataSeries3D.prototype.setMetadataAt = function (zIndex, xIndex, metadata) {
        if (!metadata) {
            return;
        }
        this.metadataProperty[zIndex][xIndex] = metadata;
     ...
     * @inheritDoc
     */
    BaseGridDataSeries3D.prototype.delete = function () {
        this.yValuesProperty = undefined;
        this.metadataProperty = undefined;
        this.yRangeCached = undefined;
    ...
    BaseGridDataSeries3D.prototype.getOptions = function (excludeData) {
        ...
        var options = {
            ...
            metadata: this.metadataProperty
        };
        return Object.assign(baseOptions, options);
    };
    BaseGridDataSeries3D.prototype.validateIndexes = function (zIndex, xIndex) {
        if (Math.round(zIndex) !== zIndex) {
            throw Error("zIndex must be an integer");
        }
        if (Math.round(xIndex) !== xIndex) {
            throw Error("xIndex must be an integer");
        }
        if (zIndex < 0 || zIndex >= this.zSizeProperty) {
            throw new Error("zIndex is out of range");
        }
        if (xIndex < 0 || xIndex >= this.xSizeProperty) {
...
// @ignore
var guardSameLengthYValuesAndMetadata = function (yValues, metadata) {
    ...
    for (var i = 0; i < yValues.length; i++) {
        Guard_1.Guard.arraysSameLengthArr([
            { arg: yValues[i], name: "yValues[".concat(i, "]") },

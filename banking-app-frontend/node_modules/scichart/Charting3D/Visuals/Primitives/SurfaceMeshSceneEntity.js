...
var SurfaceMeshSceneEntity = /** @class */ (function (_super) {
    ...
    SurfaceMeshSceneEntity.prototype.delete = function () {
        ...
        this.cellColors = (0, Deleter_1.deleteSafe)(this.cellColors);
        this.heightCoords = (0, Deleter_1.deleteSafe)(this.heightCoords);
        this.zOffsets = (0, Deleter_1.deleteSafe)(this.zOffsets);
        this.sourceRowVec = (0, Deleter_1.deleteSafe)(this.sourceRowVec);
    };
    ...
    SurfaceMeshSceneEntity.prototype.Render = function () {
        ...
        try {
            ...
            // is set to false to look like it used to be in v4
            props.m_bNormalsPS = false;
    ...
    SurfaceMeshSceneEntity.prototype.updateSeries = function () {
        ...
        if (gridDataSeries) {
            ...
            if (needUpdateMesh) {
                ...
                // Cell colors need zeroing out. All others can be resized using resizeFast
                this.heightCoords = this.getOrCreateVector(this.heightCoords, size, this.webAssemblyContext.SCRTFloatVector);
                this.zOffsets = this.getOrCreateVector(this.zOffsets, size, this.webAssemblyContext.SCRTFloatVector);
                this.cellColors = this.getOrCreateVector(this.cellColors, size, this.webAssemblyContext.SCRTUintVector); // zeroed
                this.nativeEntity.UpdateMeshesVec(this.heightCoords, this.zOffsets, this.cellColors, gridWidth, gridHeight, scrtMeshRes);
            }
            if (needUpdateHeights) {
                this.heightCoords = this.getOrCreateVector(this.heightCoords, gridWidth * gridHeight, this.webAssemblyContext.SCRTFloatVector);
                var yCalc = this.currentRenderPassData.yCalc;
                this.updateHeightMap(gridDataSeries.getYValues(), yCalc, gridDataSeries.xSize, gridDataSeries.zSize, this.heightCoords);
                this.nativeEntity.UpdateHeightCoordinatesVec(this.heightCoords, gridWidth, gridHeight);
            }
        }
    };
    /**
     * Updates the height map coordinates from the grid data series Y values
     * @param yValues Array of NumberArray Y values from the grid data series
     * @param yCalc Y-axis coordinate calculator
     * @param xSize Width of the grid (number of X points)
     * @param zSize Height of the grid (number of Z points)
     * @param heightCoords Output vector to store the calculated height coordinates
     * @protected
     */
    SurfaceMeshSceneEntity.prototype.updateHeightMap = function (yValues, yCalc, xSize, zSize, heightCoords) {
        this.sourceRowVec = this.getOrCreateVector(this.sourceRowVec, xSize, this.webAssemblyContext.SCRTDoubleVector);
        for (var z = 0, destIndex = 0; z < zSize; z++) {
            // HEAPF64.set copies js array into buffer pointer
            // MemCopy the xRow at yValues[z] fast into a temporary buffer of type SCRTDoubleVector
            // @ts-ignore
            this.webAssemblyContext.HEAPF64.set(yValues[z], this.sourceRowVec.dataPtr(0) / appendDoubleVectorFromJsArray_1.SIZEOF_NUMBER);
            // This step is the equivalent of an inner loop but performed in wasm
            // for (let x = 0; x < xSize; x++) {
            //     heightCoords.set(i++, yCalc.getCoordinate(xRow[x]));
            // }
            yCalc.nativeCalculator.GetCoordinatesVec(this.sourceRowVec, heightCoords, 0, destIndex, xSize, 0);
            destIndex += xSize;
        }
    };
